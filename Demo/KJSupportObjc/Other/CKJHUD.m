...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	CKJHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	CKJHUD *hud = [self HUDForView:view];
	if (hud != nil) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
		return YES;
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	NSArray *huds = [CKJHUD allHUDsForView:view];
	for (CKJHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.animationType = CKJHUDAnimationFade;
		self.mode = CKJHUDModeIndeterminate;
		self.labelText = nil;
...
- (void)show:(BOOL)animated {
    NSAssert([NSThread isMainThread], @"CKJHUD needs to be accessed on the main thread.");
...
- (void)showUsingAnimation:(BOOL)animated {
	...
	if (animated && animationType == CKJHUDAnimationZoomIn) {
		self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
	} else if (animated && animationType == CKJHUDAnimationZoomOut) {
	...
	if (animated) {
		...
		self.alpha = 1.0f;
		if (animationType == CKJHUDAnimationZoomIn || animationType == CKJHUDAnimationZoomOut) {
...
- (void)hideUsingAnimation:(BOOL)animated {
	...
	if (animated && showStarted) {
		...
		// in the done method
		if (animationType == CKJHUDAnimationZoomIn) {
			self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(1.5f, 1.5f));
		} else if (animationType == CKJHUDAnimationZoomOut) {
			self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
		}
