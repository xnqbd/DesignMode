...
#define kBiometricsAccountPrefix(account)  [NSString stringWithFormat:@"kSDBiometricsPrefix_%@", account]
...
+ (void)_setSwitchStatus:(BOOL)openStatus forAccount:(NSString *)account {
    ...
    [[NSUserDefaults standardUserDefaults] setObject: openStatus ? @"opened" : @"closed" forKey:kBiometricsAccountPrefix(account)];
    [[NSUserDefaults standardUserDefaults] synchronize];
}

+ (BOOL)_getSwitchStatusForAccount:(NSString *)account {
    if (WDKJ_IsEmpty_Str(account)) {
        return NO;
    }
    NSString *status = [[NSUserDefaults standardUserDefaults] stringForKey:kBiometricsAccountPrefix(account)];
    
    WDCKJ_ifDEBUG(^{
        NSLog(@"生物认证状态  %@ : %@", account, status);
    }, nil);
    
    if (WDKJ_IsEmpty_Str(status)) {
        return NO;
    }
    if ([status isEqualToString:@"opened"]) {
        return YES;
    } else {
        return NO;
    }
